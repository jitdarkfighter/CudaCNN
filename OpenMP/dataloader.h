...
MNISTData loadMNISTImages(const std::string &filename) {
    ...
    // Note: Cannot parallelize file reading operations due to sequential file access requirements
    for(int i = 0; i < number_of_images; i++) {
        data.images[i].resize(data.image_size);
        // MNIST images have one byte per pixel. can be negative as well.
        // Parallelize pixel normalization within each image
        std::vector<unsigned char> temp_pixels(data.image_size);
        file.read((char*)temp_pixels.data(), data.image_size);
        
        #pragma omp parallel for
        for(int pixel_idx = 0; pixel_idx < data.image_size; pixel_idx++) {
            data.images[i][pixel_idx] = static_cast<float>(temp_pixels[pixel_idx]) / 255.0f; // Normalize pixel values to [0, 1]
        }
    }
